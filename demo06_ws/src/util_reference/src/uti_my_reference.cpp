...
double MyReference::normalizeAngle(double angle)
{
    while (angle > PI)
        angle -= 2 * PI;
    while (angle < -PI)
...
vector<double> MyReference::calcTrackError(vector<double> robot_state)
{
    ...
    double yaw = refer_path[min_index][2]; //参考点与x轴的夹角
    double k  = refer_path[min_index][3]; //frenet斜率
    double error = d[min_index];// 车辆参考点与轨迹参考点的绝对距离
    double angle  = this->normalizeAngle(yaw - atan2(d_y[min_index] , d_x[min_index]));
    double fx = d_x[min_index], fy = d_y[min_index];
    int flag1,flag2,flag3;
    double n1,n2,n3;


    // n1 n2 n3 都是一种计算 error正负性的方式



    n1= sin(angle)*sin(refer_path[min_index][2] )   -  cos(angle)*cos(refer_path[min_index][2]) ;
    if ( (sin(angle)*sin(refer_path[min_index][2] )   -  cos(angle)*cos(refer_path[min_index][2]))  >0 )
    {
        flag1 =1;
    }
    else
 ...
 */
refTraj MyReference::calc_ref_trajectory(vector<double> robot_state,parameters param, double dl) {
