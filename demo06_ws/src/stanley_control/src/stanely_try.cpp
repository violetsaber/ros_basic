...
//        cout<<refer_path[i][0]<<" ,"<<refer_path[i][1]<<endl;
    ...
    for(int i=0;i<2000;i++){
        ...
        //参考博客中的公式
        vector<double>delta_index = stanley.stanelyControl(robot_state,refer_path);

        ugv.updateState(0,delta_index[0]); //加速度设为0，恒速
        double fx,fy;
        fx = ugv.x +   L *cos(robot_state[2]);
        fy = ugv.y + L *sin(robot_state[2])  ;
        x_.push_back(fx);
        y_.push_back(fy);
        //画图
    ...
    // save figure
    const char* filename = "./pic_save/stanley_demo4.png";
    plt::grid(true);
    plt::ylim(-5,5);
    plt::pause(0.01);
    plt::plot(refer_x,refer_y,"b--");
